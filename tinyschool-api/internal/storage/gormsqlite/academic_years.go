...
func (s *Store) ListAcademicYears(ctx context.Context, options storage.ListOptions) ([]model.AcademicYear, int64, error) {
	query := s.db.WithContext(ctx).Model(&academicYearRecord{})
	if options.Search != "" {
...
func (s *Store) AcademicYear(ctx context.Context, id string) (model.AcademicYear, error) {
	...
	err := s.db.WithContext(ctx).Preload("Segments", func(db *gorm.DB) *gorm.DB { return db.Order("position") }).First(&record, "id = ?", id).Error
	return yearModel(record), storageError(err)
}

func yearRecord(year model.AcademicYear) academicYearRecord {
	record := academicYearRecord{ID: year.ID, SchoolID: year.SchoolID, Name: year.Name, StartDate: year.StartDate, EndDate: year.EndDate, DurationDays: year.DurationDays, IsCurrent: year.IsCurrent}
...
func saveYear(tx *gorm.DB, year model.AcademicYear, create bool) error {
	if year.IsCurrent {
		if err := tx.Model(&academicYearRecord{}).Where("school_id = ? AND id <> ?", year.SchoolID, year.ID).Update("is_current", false).Error; err != nil {
			return err
		}
	}
	...
	result := tx.Model(&academicYearRecord{}).Where("id = ?", year.ID).Updates(map[string]any{
...
func (s *Store) CreateAcademicYear(ctx context.Context, year model.AcademicYear) (model.AcademicYear, error) {
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error { return saveYear(tx, year, true) })
...
func (s *Store) UpdateAcademicYear(ctx context.Context, year model.AcademicYear) (model.AcademicYear, error) {
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error { return saveYear(tx, year, false) })
	return year, storageError(err)
}

func (s *Store) DeleteAcademicYear(ctx context.Context, id string) error {
	result := s.db.WithContext(ctx).Delete(&academicYearRecord{}, "id = ?", id)
	if result.Error != nil {
